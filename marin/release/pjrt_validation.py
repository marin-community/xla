...
def fetch_wheel(url: str, sha256: str, destination: Path) -> Path:
    ...
    and rejects ``...0.11.1%2Bmarin...`` as invalid.
    """
...
def install_runtime(config: dict, wheel: Path, python: str) -> None:
    """Install the stock sibling generation from PyPI, then overlay the candidate wheel.
    ...
    """
    siblings = f"jax[cuda13]=={config['jax_version']}"

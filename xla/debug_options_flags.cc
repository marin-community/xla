...
DebugOptions DefaultDebugOptionsIgnoringFlags() {
  ...
  opts.set_xla_gpu_experimental_ragged_all_to_all_ctas_per_sm(0);
...
void MakeDebugOptionsFlags(std::vector<tsl::Flag>* flag_list,
  ...
  flag_list->push_back(tsl::Flag(
      ...
      int32_setter_for(
          &DebugOptions::
              set_xla_gpu_experimental_ragged_all_to_all_ctas_per_sm),
      debug_options
          ->xla_gpu_experimental_ragged_all_to_all_ctas_per_sm(),
      "Resident CTAs per SM for the ragged-all-to-all device kernel's grid. 0 "
      "keeps the kernel's own default, as does any value above it or below "
      "1."));
  flag_list->push_back(tsl::Flag(
